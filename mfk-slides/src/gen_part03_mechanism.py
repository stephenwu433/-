...
def slide_01():
    ...
    header(
        d,
        "03 / MECHANISM  ·  1 of 5",
        "推演机制：方向不是「感觉出来的」，是过闸推出来的",
        "研究问题：为什么这个方向有资格成为 MFK 的长期资产？先讲规则，再讲结果。",
    )
...
def slide_02():
    ...
    header(
        d,
        "03 / CONFLICT ENCODING  ·  2 of 5",
        "闸门 A：冲突如何被编码成洞察簇",
        "机制：标签词典里的 CF（核心冲突）不是装饰分类——它规定「什么张力才有资格进战场」。",
    )
...
def slide_03():
    ...
    header(
        ...
        "03 / INS → DIR  ·  3 of 5",
        "转化机制：一条洞察怎样变成一条候选方向",
        "规则：每个 DIR 必须绑定唯一主冲突/需求；写清「美学命题 + MFK可兑现资产 + 反例风险」，否则不许进压力测试。",
    )
...
def slide_04():
    ...
    header(
        d,
        "03 / GATE MATRIX  ·  4 of 5",
        "闸门 B+C：筛选矩阵——谁被杀死，谁被留下",
        "两道硬闸并行：CHK（空话 / 可替换 / 证据）× 五维压力测试。PoC 全部「暂缓」≠ 方向不成立，只说明规模化未证。",
    )
...
def slide_05():
    ...
    header(
        d,
        "03 / WHY BRD-02  ·  5 of 5",
        "闸门 D：为什么主推是「精准的感性」，不是证据最多的那条",
        "决策公式：主推 = 过闸方向中「稀缺 × 难复制」最高者；证据条数只决定是否进漏斗，不决定主推。",
    )
    ...
    # Bottom conclusion
    rr(d, [MX, 1530, W - MX, 1920], SOFT_G, 14)
    d.text((MX + 40, 1570), "第三章收束：方向如何被推出来（完整机制）", font=F["h2"], fill=INK)
    closing = [
        "① 配置只研香水，竞品对照 Le Labo / Byredo（P-001）",
        "② 语料用 26 标签编码；CF 冲突标签决定谁能进战场（闸门 A）",
        "③ 5 个洞察簇中，4 个生成 DIR；N04 因「可被档期消化」不生成（转化规则，非空话闸）",
        "④ CHK+五维：空话杀 DIR-01；可替换杀 DIR-04；留下 02/03（闸门 B+C）",
        "⑤ 主推不看条数，看稀缺×难复制 → BRD-02「精准的感性」；BRD-01 作旅程确认（闸门 D）",
        "⑥ 边界诚实：方向过闸成立，规模化仍「PoC 暂缓」——交给 PART 04–06 试点验证",
    ]
    for i, line in enumerate(closing):
        d.text((MX + 40, 1650 + i * 40), line, font=F["body"], fill=INK)

    footer(d, "Source: BRD表状态字段（主推/确认）+ CHK-002/003 + 资产四问评分；对齐 PART01 场景层 / PART02 选择权威缺口")
